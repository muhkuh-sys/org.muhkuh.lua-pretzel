...
--- Get the MySQL server version in one integer.
-- This works like the API function mysql_get_server_version described here:
-- https://dev.mysql.com/doc/refman/8.0/en/mysql-get-server-version.html
function Pretzel:__getServerVersion(tConnection)
  -- Is a cached server version available?
  local ulServerVersion = self.__ulServerVersion
  if ulServerVersion==nil then
    -- No cached version available, start a new query.
    local strQuery = 'SELECT VERSION()'
    local tCursor, strError = tConnection:execute(strQuery)
    if not tCursor then
      error(string.format('Failed to execute the query "%s": %s', strQuery, strError))
    else
      local aRow = tCursor:fetch({}, "a")
      if not aRow then
        error('Failed to get the server version.')
      else
        -- The result of the query has the form major.release.sublevel . An
        -- optional description is appended with a dash.
        -- Extract the 3 version components and build a number like mysql_get_server_version does.
        local strVersion = aRow['VERSION()']
        local uiMaj, uiRel, uiSub = string.match(strVersion, '(%d+)%.(%d+)%.(%d+)')
        if uiMaj==nil then
          error(string.format('Failed to parse the server version: "%s"', strVersion))
        end
        print(string.format('Server version: %d.%d.%d', uiMaj, uiRel, uiSub))
        ulServerVersion = 10000*uiMaj + 100*uiRel + uiSub
        self.__ulServerVersion = ulServerVersion
      end
    end
...
function Pretzel:__get_group_idx(tConnection, strGroup)
  ...
  -- The table holding the MAC groups was named "groups" back when MySQL 5.7
  -- was still young and fresh. This became a reserved keyword with MySQL 8.0.2
  -- and the table was renamed to "macgroups".
  local strGroupsTable = 'groups'
  local ulServerVersion = self:__getServerVersion(tConnection)
  if ulServerVersion>=80002 then
    strGroupsTable = 'macgroups'
  end
  local strQuery = string.format("SELECT idx FROM %s WHERE name='%s'", strGroupsTable, strGroup)
  local tCursor, strError = tConnection:execute(strQuery)
  if not tCursor then
...
function Pretzel:request(dev_par, cnt)
  ...
  if tResponse==nil then
    error('Failed to request a new MAC.')
  else
...
function Pretzel:get_board_info(strGroup, ulManufacturer, ulDeviceNr, ulSerialNr)
  ...
  if not tEnv then
    ...
    local tConnection, strError = tEnv:connect(self.CFG_Pretzel_DB, self.CFG_Pretzel_User, self.CFG_Pretzel_Pwd, self.CFG_Pretzel_Server)
    if not tConnection then
      print("failed to connect to the database: ".. strError)
    else
